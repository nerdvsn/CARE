...
class BaseProcess():
    ...
    def BandPass(self, sensor_mat, sensor_at):
        # Clip auf 37°C (nicht auf ambient!)
        matrix1 = np.clip(sensor_mat, None, self.temperature_upper_bound)
        return matrix1
...
class DetectingProcess():
    ...
    def Forward(self, frame):
        ...
        """
        s = self.expansion_coefficient * self.valid_region_area_limit  # κ × θ = 5 × 20 = 100
        box_size = s + 1 if s % 2 == 0 else s  # Ensure odd: 101
        
        # Alternative (falls 101 zu groß):
        # box_size = min(s + 1 if s % 2 == 0 else s, 51)  # Cap bei 51
        ...
        # Pixel limit für Blob-Removal
        # Paper: "minimum size for a single user's region κ to 5"
        # Nach Expansion: 5 * (20*20) = 2000 Pixel
        expansion_coeff = self.expansion_coefficient * self.expansion_coefficient
        pixel_limit = expansion_coeff * self.valid_region_area_limit  # 2000
