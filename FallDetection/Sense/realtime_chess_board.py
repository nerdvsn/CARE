...
def apply_inferno_colormap_with_chessboard(matrix, subpage_type, expansion_coefficient, temp_min=15.0, temp_max=45.0):
    ...
    if subpage_type == 0:
        # Subpage 0 gelesen -> Inverse-Maske (Subpage 1) ist nicht aktuell
        mask = CHESSBOARD_MASK_INV
    else:
        # Subpage 1 gelesen -> Normale Maske (Subpage 0) ist nicht aktuell
    ...
    try:
        while True:
            ...
            for idx, (x, y, w, h) in enumerate(valid_bboxes):
                ...
                # Bounding Box zeichnen (Grün, gut sichtbar auf Inferno)
                cv2.rectangle(
                    ira_colored, 
                    (int(x), int(y)), 
                    (int(x + w), int(y + h)), 
                ...
                cv2.putText(
                    ...
                    fontScale=0.8, 
                    color=(0, 255, 255),  # Cyan (gut sichtbar auf Inferno)
                    thickness=2
                )
            ...
            # Info-Overlay
            info_text = f"Subpage: {subpage_type} | Chessboard: {'ON' if show_chessboard else 'OFF'} | Persons: {len(valid_bboxes)}"
            cv2.putText(
                ira_colored, 
                info_text, 
                (10, 25), 
            ...
            if key in {27, ord('q')}:  # ESC oder 'q'
                ...
                print(f"Chessboard: {'ON' if show_chessboard else 'OFF'}")
