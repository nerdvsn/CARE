...
def getBoxLinesCoordsDynamic(x, y, z, xrad=0.25, yrad=0.25, zrad=1):
    # Dynamic scaling based on z value
    if z > 0:
        # More wide than tall when z > 0
        # Increase width (x dimension) with z magnitude
        width_factor = 1 + z * 0.5  # Adjust multiplier for sensitivity
        zl = z - (zrad * width_factor)
        zr = z + (zrad * width_factor)
        # Keep height normal or slightly reduced
        yl = y - yrad
        yr = y + yrad
    else:
        # More tall than wide when z <= 0
        # Increase height (y dimension) with |z| magnitude
        height_factor = 1 + abs(z) * 1.5  # Adjust multiplier for sensitivity
        yl = y - (yrad * height_factor)
        yr = y + (yrad * height_factor)
        # Keep width normal or slightly reduced
        zl = z - zrad
        zr = z + zrad
    
    # Z dimension (becomes x in sideways orientation)
    xl = x - xrad
    xr = x + xrad
